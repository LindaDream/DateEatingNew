...
@implementation YChatRecieveViewCell
...
-(UIImageView *)backImgView{
    if (!_backImgView) {
        if (_messageLabel.text.length * 16 <= _messageLabel.width) {
            _backImgView = [[UIImageView alloc] initWithFrame:CGRectMake(_headImgView.width + 10, 5, self.messageLabel.text.length * 16 + 22, _messageLabel.height + 20)];
            CGPoint point = _backImgView.center;
            point.y = self.contentView.center.y;
            _backImgView.center = point;
        }else{
            _backImgView = [[UIImageView alloc] initWithFrame:CGRectMake(_headImgView.width + 10, 5, 190, _messageLabel.height + 20)];
            CGPoint point = _backImgView.center;
            point.y = self.contentView.center.y;
            _backImgView.center = point;
        }
